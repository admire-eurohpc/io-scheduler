...
ADM_adhoc_context(hg_handle_t h){
    ...
    if ((strcmp(in.context, "in_job:shared")) == 0 || (strcmp(in.context, "in_job:dedicated")) == 0 || (strcmp(in.context, "separate:new")) == 0
...
/**
 * Transfers the dataset identified by the source_name to the storage tier defined by 
 * destination_name, and apply the provided constraints during the transfer. This function returns a 
 * handle that can be used to track the operation (i.e., get statistics, or status).
 *
 * @param in.source A source_location identifying the source dataset/s in the source storage tier.
 * @param in.destination A destination_location identifying the destination dataset/s in its desired location in a
 * storage tier.
 * @param in.qos_constraints A list of qos_constraints that must be applied to the transfer. These may not exceed the
 * global ones set at node, application, or resource level (see Section 3.4).
 * @param in.distribution A distribution strategy for data (e.g. one-to-one, one-to-many, many-to-many)
 * @param in.job_id A job_id identifying the originating job.
 * @param out.transfer_handle A transfer_handle allowing clients to interact with the transfer (e.g. wait for its
 * completion, query its status, cancel it, etc.
 * @return out.ret Returns if the remote procedure has been completed successfully or not.
 */
static void 
ADM_transfer_dataset(hg_handle_t h){
    ...
    LOGGER_INFO("remote_procedure::ADM_transfer_dataset({},{},{},{},{})",
                in.source, in.destination, in.qos_constraints, in.distribution, in.job_id);

    if (in.source != nullptr && in.destination != nullptr && in.qos_constraints != nullptr && in.distribution != nullptr && in.job_id >= 0){
       out.ret = 0;
        LOGGER_INFO("remote_procedure::ADM_transfer_dataset not null ({},{},{},{},{})",
                in.source, in.destination, in.qos_constraints, in.distribution, in.job_id);
 ...
 * @param in.info An opaque inf o argument containing information about the dataset (e.g. its lifespan, access
 *  methods, intended usage, etc.).
 * @param in.job_id A job_id identifying the originating job.
 * @param out.status A status code determining whether the operation was successful.
 * @return out.ret Returns if the remote procedure has been completed successfully or not.
 */
static void 
 ...
 * @param in.resources An opaque resources argument containing information about the I/O resources to modify (e.g. 
 * number of I/O nodes.).
 * @param in.job_id A job_id identifying the originating job.
 * @param out.status A status code determining whether the operation was successful.
 * @return out.ret Returns if the remote procedure has been completed successfully or not.
...
ADM_set_io_resources(hg_handle_t h){
    ...
    LOGGER_INFO("remote_procedure::ADM_set_io_resources({},{},{})",
